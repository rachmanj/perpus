        ...
        <div class="box-body">
            <form action="{{ route('admin.book.update', $book) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group @error('description') has-error @enderror">
                    ...
                    <textarea name="description" rows="3" class="form-control">{{ $book->description ?? old('description') }}</textarea>
                    @error('description')
                        <span class="help-block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group @error('qty') has-error @enderror">
                    ...
                    <input type="text" name="qty" class="form-control" placeholder="Masukkan jumlah" value="{{ $book->qty ?? old('qty') }}">
                    @error('qty')
                        <span class="help-block">{{ $message }}</span>
                    @enderror
                </div>
